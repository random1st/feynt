...
Четыре, и список закрыт по измеримому правилу: модель попадает в него, только если
спекуляция её реально ускоряет. Каждый кандидат прогонялся против собственного обычного
декода, на прогретом кэше, дважды.

| Модель | Обычный декод | Со спекуляцией | Ускорение | Принято за раунд |
|---|---:|---:|---:|---:|
| Qwen3.6-35B-A3B (MoE) | 73 | **121** | 1.7x | 7.08 |
| Qwen3.8-27B Uncensored | 14.6 | 40 | **2.7x** | 4.10 |
| Qwen3.6-27B | 17.2 | 41 | 2.4x | 4.86 |
| Qwen3.8-27B | 18.6 | 35 | 1.9x | 3.77 |

Токенов в секунду, M3 Max. MoE самая быстрая в абсолюте: на токен читается ~3B параметров
из 35B, а декод упирается ровно в это чтение.

Поколение 3.5 из каталога убрано: его драфтеры — DFlash 1, без селектора кандидатов, и
приёмка это показывает. У Qwen3.5-9B ускорение оказалось ровно нулевым (57 против 56),
у MoE 3.5 — 1.1x. Модель, которой спекуляция не помогает, продавать нечем.
